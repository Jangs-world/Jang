...
$(function () { /////////// jQB //////////////////
  ...
  /* 
      [ 입력폼의 유효성 검사 ] 
      - 검사원리 :
      입력창에 클릭 또는 탭하여 입력가능상태(커서깜빡)를 포커스 상태라고함.
      이벤트로는 focus 이벤트임!
      이 포커스 상태에서 다른 부분을 클릭(탭)되면 포커스가 풀리게된다.
  ...
  */
  ...
  $("input[type=text][id!=email2][class!=search],input[type=password]")
    .blur(function () {
      ...
      // trim()메서드 : 앞뒤공백제거 + 공백만있어도 제거

      if (cid === "mnm") // 이름일 경우 앞뒤공백만 제거
        cv = $(this).val().trim();
      else // 그밖의 경우는 모든공백제거
      ...
      // 4. 아이디일때 검사하기 ////////////////
        ...
        // 결과가 false일 경우 메세지 띄우기
        if (!res) { // !(NOT연산자)로 결과 반대로!
          $(this).siblings(".msg")
            .text("영문자로 시작하는 6~20글자 영문자/숫자")
            .removeClass("on"); // 글자색 변경제거
  ...
  $("#email1, #email2").on("keyup", function () {
    ...
    // 조건연산자로 선택박스값 또는 직접입력각ㅄ을 할당한다!
    // 비? 집: 놀이동산;

    // 4. 선택박스의 선택값이 "free"(직접입략)이면 이메일 뒷주소 변경
  ...
  $("#btnj").click(function (e) { // 서브밋버튼 클릭시
    ...
    if (pass) { /// 통과시!
      ...
      /* 
          ...
          $.post(전송할 페이지, 전송할 데이터, 전송 후 실행함수)
