...
// 처음 페이지 로딩시 최초 업데이트 함수 호출!
reFn();

// 윈도우 사이즈 변경시 업데이트함수 호출!
$(window).resize(reFn);
// resize() 메서드 : 화면크기가 변경될 때 발생하는 이벤트와 함수를 연결
...
$(function () { ////////// jQB //////////////////////
  ...
  // 브라우저 스크롤 위치 캐싱(즉, 위치기억)에 의해서
  // 그 위치에 있게되므로 강제로 위로 보내는 코드를 넣어준다!
  // CSS로 넣어주면 캐싱 엎어써서 소용이 없다!
  // 그래서 animate로 넣어줌
  $("html, body").animate({
    scrollTop: "0px"
  }, 100);
  ...
  */
  ...
  let scTop;
  // 스크롤 실행상태변수(상단영역)
  let scSts = 1; // 실행 전1, 실행 후 0
  // 스크롤 실행상태변수2(top버튼)
  let scSts2 = 1; // 실행 전1, 실행 후 0
  // 상단영역요소
  let tm = $("#top");
  // 위로이동버튼 요소
  let tbtn = $(".tbtn");
